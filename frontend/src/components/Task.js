...
function Task() {
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`https://progy.onrender.com/project/${projectId}`)
                .then((res) => {
    ...
    const updateTodo = (data) => {
        axios.put(`https://progy.onrender.com/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`https://progy.onrender.com/project/${projectId}/task/${taskId}`)
            .then((res) => {
    ...
    return (
        <div className='px-12 py-6 w-full'>
            <div className="flex items-center justify-between mb-6">
                ...
                <BtnPrimary onClick={() => setAddTaskModal(true)}>Add A Task</BtnPrimary>
            </div>
            <DragDropContext
                                        ...
                                        {(provided, snapshot) => {
                                            return (
                                                <div
                                                    ...
                                                    className={`min-h-[530px] pt-4 duration-75 transition-colors border-t-2 border-gray-400 ${snapshot.isDraggingOver && 'border-gray-600'}`}
                                                >
