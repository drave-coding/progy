...
const ProjectDropdown = ({ id, navigate }) => {
    ...
    const handleDelete = async () => {
        try {
            const data = await axios.delete(`https://progy.onrender.com/project/${id}`)
                ...
                <Popover.Panel className="absolute z-10 left-0">
                    <div className="w-48 bg-white rounded-md border shadow select-none p-1 divide-y">
                        <div className='py-[3px]'>
                            <button onClick={() => setModalState(true)} className='transition-colors duration-75 flex w-full items-center rounded-md px-2.5 py-2 text-sm space-x-2.5 text-slate-500 hover:bg-gray-500 hover:text-gray-100'>
